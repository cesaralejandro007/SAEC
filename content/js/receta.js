...
document.getElementById("nuevo").onclick = function () {
  ...
  $(document).ready(function() {
    ...
    $('#abreviatura').attr('maxlength', 15);
    $('#abreviatura').on('keyup', function() {
      validarabreviatura();
    }).on('keypress', function(event) {
        validarEntrada(event, /^[0-9]*$/);
    });
    $('#descripcion').attr('maxlength', 200);
    $('#descripcion').on('keyup', function() {
      validardescripcion();
    });
  
    $('#enviar_receta').on('click', function() {
  ...
  function validarabreviatura() {
    ...
    var abreviaturaRegex = /^[0-9]*$/;
  
    if (abreviatura.val().trim() === '' || !abreviaturaRegex.test(abreviatura.val())) {
      abreviatura.addClass('error-input');
      $('#sabreviatura').addClass('error-message').text('El precio ingresado no es válido');
    } else {
  ...
  function validardescripcion() {
    ...
    var descripcionRegex = /^[a-zA-Z0-9\s.,#áéíóúÁÉÍÓÚüÜñÑ-]+$/;
  
    if (descripcion.val().trim() === '' || !descripcionRegex.test(descripcion.val())) {
        descripcion.addClass('error-input');
        $('#sdescripcion').addClass('error-message').text('La descripción ingresada no es válida.');
    } else {
        descripcion.removeClass('error-input');
        $('#sdescripcion').removeClass('error-message').text('');
    }
  }
  ...
  if ($("#snombre").text() !== '') {
    ...
    $('#abreviatura').focus();
  }else if ($("#sdescripcion").text() !== '') {
  mensajeError += $("#sdescripcion").text();
  $('#descripcion').focus();
  }
